...
# NOTE: Because a fancy subclass-based approach was throwing some (potentially spurious) warnings,
#       here's a dumb varaint that just copes the whole class ;'(.
...
class TransformerDecoderSwiGLU(keras.layers.Layer):
	"""Transformer decoder.
	...
	def call(
		...
		if has_cross_attention and (has_self_attention_cache != has_cross_attention_cache):
			raise ValueError(
	...
	def _compute_self_attention_mask(
		...
		decoder_mask = merge_padding_and_attention_mask(decoder_sequence, decoder_padding_mask, decoder_attention_mask)
		if use_causal_mask:
			...
			causal_mask = compute_causal_mask(
				...
				input_length,
				output_length,
				(0 if self_attention_cache_update_index is None else self_attention_cache_update_index),
			)
